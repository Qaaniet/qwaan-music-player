...
function ChromeButton({
  ...
  return (
    <button
      ...
      className={`group inline-flex h-10 w-12 items-center justify-center rounded-[4px] transition-colors duration-150 ${
        danger
          ? "text-white/78 hover:bg-[#c42b1c] hover:text-white active:bg-[#a52417]"
          : "text-[var(--win-text-secondary)] hover:bg-[var(--win-pane-hover)] hover:text-[var(--win-text)] active:bg-[var(--win-pane-active)]"
      }`}
    >
...
export function WindowChrome() {
  ...
  return (
    <header className="win-app-shell relative z-[80] h-11 select-none border-b border-[var(--win-border)] backdrop-blur-xl">
      ...
      <div
        ...
        className="flex min-w-0 flex-1 items-center gap-2.5 px-4 select-none"
      >
        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-[4px] bg-[linear-gradient(135deg,rgba(255,255,255,0.94),rgba(var(--win-accent-rgb),0.82))] text-[10.5px] font-semibold text-[#09111d]">
          Q
        </div>
        <div className="truncate text-[11.5px] font-medium tracking-[-0.01em] text-[var(--win-text)]">
          Qwaan Music Player
        </div>
      </div>
